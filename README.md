...
- 画面設計書（optional）: `jskim spec collect` / `jskim spec build` / **`jskim spec dev`**（収集・viewer build・同一 port の `/spec/` 自動更新）。`spec dev` には Device Capture 収集 API（PC/SP Preview 画像）もあります（Viewer タブ UI は後続）。通常の `jskim dev` は Screen Spec を自動実行しません。
