...
function currentEntry(): ReferenceImageManifestEntry {
  return {
    status: 'current',
...
describe('ReferenceImagePanel', () => {
  it('missing + editable では追加ボタンのみ表示する', () => {
  ...
  it('read-only では追加/置き換え/削除ボタンを出さない', () => {
    ...
    const current = mount(ReferenceImagePanel, {
      props: baseProps({ editable: false, reference: currentEntry() }),
    });
    expect(
