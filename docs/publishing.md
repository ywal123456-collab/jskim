...
companion: @ywal123456/jskim-screen-spec（optional）
creator: create-jskim
CLI binary: jskim / create-jskim
license: MIT
```

初回公開（v0.1.0）は完了しています。以降の patch / minor release でも、同じ手順で **engine → companion → creator** の順に公開します。
...
companion package: @ywal123456/jskim-screen-spec
...
scoped package（engine / companion）の publish では手動コマンドの `--access public` を省略しないでください。
companion は `prepack` で `dist` をビルドします（publish 前に別途 `npm run build` が必要な場合もあります）。
...
Engine を先に publish します。companion は peer で `@ywal123456/jskim` を参照し、生成プロジェクトの `package.json` が engine を参照するため、**engine → companion → creator** の順にします。

1. version を決定する
2. リポジトリ root で `npm test` を実行する
3. engine / companion / creator の `npm pack` を確認する
...
8. **companion を publish** する（`--access public`。`prepack` が `dist` をビルド）
9. registry から companion をインストールできることを確認する
10. creator を publish する
11. `npm create jskim@latest` を検証する
12. Git tag と GitHub Release を作成する
...
Companion（`jskim-screen-spec/`）:

```powershell
cd jskim-screen-spec
npm.cmd publish --dry-run --access public --registry=https://registry.npmjs.org
npm.cmd publish --access public --registry=https://registry.npmjs.org
```

Registry / companion 確認:

```powershell
npm.cmd view @ywal123456/jskim-screen-spec name version dist-tags --registry=https://registry.npmjs.org
npm.cmd install --save-dev @ywal123456/jskim-screen-spec --registry=https://registry.npmjs.org
```
