...
**Phase 7B-2A で追加実装済みなのは** Description Schema `1.1`（`itemOrder: string[]` 必須）、`1.0` / `1.1` 両方の読込、Viewer 保存時の `1.1` への lazy migration、PUT での項目追加（既存 ID の変更・削除は拒否）、Viewer での手動項目追加 UI、上下ボタンによる並び替え、Collector merge 時の `itemOrder` 維持・末尾追加です。
新規画面の POST 作成は `schemaVersion "1.1"` ＋ `itemOrder: []` で作成します。
既存の sample / fixture の Description JSON（`1.0`）は書き換えていません（lazy migration のため）。

項目の **削除・複製・drag-drop 並び替え**、画面の複製・削除・アーカイブ、Figma 連携は未実装です（Phase 7B-2B / 7B-3 以降）。
...
2. Preview は無い状態で表示される（**No Preview**）。項目の追加・並び替えは可能（6章・Phase 7B-2A）。削除・複製は未実装
...
## 6. 項目 CRUD（Phase 7B-2A・追加/並び替えのみ実装済み）
...
削除・複製・drag-drop 並び替えは Phase 7B-2A の範囲外です（6.5 / 6.6 章参照）。
...
- 既存ファイルがある場合: **既存 item ID の集合は新しい item ID 集合に含まれる必要がある**（`oldIds ⊆ newIds`）。新規追加は許可し、削除・rename は拒否する
- 既存ファイルが無い場合（IMPLEMENTATION_ONLY 初回 PUT）: collected item ID の集合が新しい item ID 集合に含まれる必要がある（追加は許可）
...
Viewer では上下ボタン（`ItemDescriptionTable` の「上へ」「下へ」）で並び替えます。**delete / 複製 / drag-drop 並び替えは未実装です。**
...
| 論点 | 現時点の方針 |
|--------|----------|
| Description 側のみの項目削除 | 未実装（Phase 7B-2A では追加・並び替えのみ） |
| 実装側に残っている ID の削除 | 削除しても、次の collect で placeholder として **再追加**される（既存 merge の継続） |
| 削除に伴う validation | 未実装（削除 API 自体が無い） |
...
項目の追加・並び替えは Phase 7B-2A で実装済みです（6章）。削除・複製は未実装です。
...
PUT での item 集合変更検証（既存 ID の変更・削除を拒否、追加のみ許可。oldIds ⊆ newIds）
itemOrder ↔ items のキー集合 bijection 検証
...
6. **項目の追加・並び替えは whole-document PUT のみ（7B-2A・実装済み）。削除・複製は未実装**
...
11. **実装順: 7B-1 → 7B-2A（完了）→ 7B-2B（項目削除・複製）→ 7B-3 → 7C → 7D**
