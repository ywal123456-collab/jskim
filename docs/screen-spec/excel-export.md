# Screen Spec Excel Export 設計（Phase 7E-0）
...
- Screen Spec の確定済みデータ（Description / Source / Manifest / Reference Image / Device Capture）から、**顧客・社内へ渡せる日本語の画面設計書 workbook** を生成する。
- 生成物は **プロジェクトあたり 1 workbook** を基本とする。
- 既存の手作業文書と **見た目の共通言語**（A4 横、色、表見出し）を共有しつつ、**クリーンな新規 workbook** をゼロから組み立てる。

## 2. スコープ（v1）

- `jskim spec export`（`--format xlsx`）による片方向 Export
- 改訂履歴シート + 画面ごとの画面設計シート
...
| Source | 実装横 `*.spec.json` | `screen.id` / `path` / `states` / `interactions` |
| Viewer ScreenData | `spec/{project}/dist/data/screens/*.json` | collect+build 成果 |
| Manifest | `spec/{project}/dist/data/manifest.json` | 画面一覧・status |
| Snapshot HTML | `spec/{project}/src/snapshots/...` および dist | |
| Reference Image | `spec/.../reference-images/{screenId}/{pc\|sp}/` | meta + PNG |
| Device Capture | state 付随 `deviceCaptures.pc/sp` | meta + PNG |
...
- **非公開**: Figma token、fileKey、nodeId、signed URL
...
| Excel 領域 | 判定 | 根拠 |
|------------|------|------|
| 改訂履歴（export メタ） | 一部生成可能 | project / package version / 任意で時刻。著者・承認者は無し |
| 基本情報（画面名, screenId, path, status） | 正確に生成可能 | Manifest + ScreenData |
...
1. `改訂履歴`（先頭固定）
2. 各画面 `画面設計(<画面名>)`（画面順）
3. **処理設計シートは v1 では生成しない**（データが無く、空 IPO は誤解を招くため）

将来、transition / API / validation の schema 拡張後に `処理設計(<画面名>)` を追加する。

### 6.3 画面順

1. Manifest `screens` 配列順（build 時の決定順）をそのまま使用  
2. Manifest が無い場合は export 失敗（推測ソートしない）
...
| 列（案） | 内容 |
|----------|------|
| 版 | 連番または `export` 固定ラベル（v1 は単一行で可） |
| 日時 | オプション。`--timestamp` 指定時のみ ISO（ローカル表示は JST 固定文字列）。未指定時は空または `（未記録）` |
| 内容 | `JSKim Screen Spec Excel Export` + engine version + companion version |
| 備考 | `projectName`、任意で Git commit（`--git-rev` かつ取得成功時のみ。失敗してもエラーにしない） |

- 著者・承認者・顧客名は **入れない**
- deterministic を優先する場合は日時列を空にし、version のみ記録（CLI 既定は **日時なし** を推奨）
...
| 項目 | v1 契約案 |
|------|-----------|
| 入力形式 | PNG（現行 Reference / Capture 契約に合わせる）。JPEG は受けても再エンコードせず **非対応エラー** でも可 |
| 最大辺 | 4096 px（超えは縦横比維持で縮小して埋め込み） |
| 最大バイト | 8 MiB / 画像（超えは `SPEC_EXCEL_EXPORT_IMAGE_TOO_LARGE`） |
| 縦長ページ | 幅をシート列幅に合わせ縮小。分割印刷は v1 非対応（1 画像 1 アンカー） |
| workbook 目安 | 推奨合計 32 MiB 未満（超過は警告ログのみ。失敗にはしない） |
| 秘密 | fileKey / nodeId / token / signed URL をセル・メモ・ファイル名に出さない |
| パス | media は workbook 内埋め込みのみ。外部リンク画像は使わない |
...
| 候補 | 評価 |
|------|------|
| `jskim spec export <project> --format xlsx` | **採用**。将来 CSV/PDF 等へ `--format` 拡張しやすい |
...
実装時は `export` を追加し、option として `--format` / `--output` / `--force` /（任意）`--timestamp` / `--git-rev` を許可する。
...
- **既定: しない**。既存の `spec/{project}/dist`（manifest + screens + 画像）を読む。
- dist が無い / 古い場合はエラーで案内: `jskim spec collect` → `jskim spec build`（または `spec dev`）
- `--from-src` のような再 collect は v1 非対応（誤って長い Playwright を踏まない）

### 10.6 複数 project

v1 は 1 回の呼び出しで 1 project。`--all` は入れない（build の `--all` と意味が違うため別途検討）。
...
- `export/` は gitignore 推奨（実装 Phase で template / docs に記載）
- directory が無ければ作成する
...
同一の `spec/{project}/dist` 入力（＋同一 CLI オプション）に対し:

- シート名集合と順序
- 各セルの文字列・数値の意味
- 画像の採用ファイル（path / revision）
- 項目行順（itemOrder）
...
- 既定: 改訂履歴に時刻を書かない（再現性優先）
- `--timestamp` で明示したときのみ記録
...
| `SPEC_EXCEL_EXPORT_SPEC_NOT_BUILT` | 画面設計書の build 結果がありません。先に `jskim spec collect` と `jskim spec build` を実行してください。 |
| `SPEC_EXCEL_EXPORT_MANIFEST_INVALID` | manifest を読めません。spec build 結果を確認してください。 |
...
- 空に近い project / 画面 0 件
- PC only / SP only / both（Reference・Capture 各組合せ）
...
- Windows Excel または LibreOffice で A4 横・画像・日本語折り返しを目視（Phase 7E-3）

---

## 18. 実装Phase

### Phase 7E-1 — domain + renderer

- workbook domain model（純関数）
- ExcelJS 導入（companion dependency）
- 改訂履歴 + 画面設計シート + スタイル + 画像埋め込み
- semantic unit tests  
**完了条件**: 一時 directory に xlsx を生成し、再読込 assertion が CI で green。CLI 未接続で可。

### Phase 7E-2 — CLI + I/O

- `parse-cli-args` に `spec export`
- companion 公開 API + root command runner
- 既定 output / `--output` / `--force` / atomic rename
- エラーコードと日本語メッセージ
- package boundary / files 回帰テスト  
**完了条件**: 一時 workspace で `jskim spec export sample --format xlsx` が成功。既存 spec コマンド回帰が green。

### Phase 7E-3 — 画像高度化 + 配布検証 + 文書

- 長尺画像の縮小、stale 注記、workbook サイズ警告
- packed consumer + Windows Excel 目視
- companion / root README の利用手順（日本語）  
**完了条件**: packed 検証手順が文書化され、秘密スキャン付きテストが通る。

（調査により 7E-1 で画像基本まで含め、7E-3 を薄くしてもよい。）

---

## 19. 未決事項（ユーザー確認）

1. 改訂履歴の **既定を日時なし** でよいか（推奨: はい）
2. `--include-excluded` を v1 に入れるか（推奨: 後回し）
3. Device Capture が state 複数あるとき、Export に載せる state は **viewer 先頭 visible** でよいか
4. `export/` を create-jskim template の `.gitignore` に最初から入れるか
5. 処理設計シートを「将来用の空見出し 1 枚」として出す需要があるか（推奨: 出さない）
