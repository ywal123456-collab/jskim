...
- **Viewer UI の除外 / 復元ボタンは未実装**（phase 7B-2C-2）。保存契約・Collector・PUT validation のみ 7B-2C-1 で実装済み
...
### 実装済み（phase 7B-2C-1 収集項目の設計対象除外 — 保存 / Collector / PUT）
...
- **未実装（7B-2C-2）**: Viewer の除外 / 復元 UI、除外一覧表示
