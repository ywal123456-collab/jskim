...
- Feature Group schema / ローカル版管理 object store（Phase **7E-1 実装済み**・CLI/UI 未提供。設計: [local-version-control.md](./local-version-control.md)）
- snapshot/status/diff/stage/commit / Revision API / 改訂履歴 modal / Feature Viewer（Phase 7E-2 以降・未実装）
...
| `local-version-control.md` | 画面中心モデル・Feature Group・ローカル版管理設計（Phase 7E-0・未実装） |
