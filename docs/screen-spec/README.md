...
想定 CLI（未実装。現状は companion の npm scripts）:

```bash
npm --prefix jskim-screen-spec run build:sample
jskim spec collect sample   # 未実装
jskim spec build sample     # 未実装
jskim dev sample            # /spec/ 統合は未実装
```
...
- `/spec/` を同一 port で提供する `jskim dev` 統合 / `jskim spec` CLI
...
7. Playwright collector / `jskim spec` CLI / `jskim dev` の `/spec/` 統合
