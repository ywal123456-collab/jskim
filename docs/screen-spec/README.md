...
### 設計のみ（未実装）: Phase 7D Figma Frame Import

詳細方針: [figma-frame-import.md](./figma-frame-import.md)

- Figma Frame を PNG export し、既存 Reference Image 契約へ保存する方針（専用ストアは作らない）
- 初期は Personal Access Token（環境変数 `JSKIM_FIGMA_TOKEN`）と `jskim spec dev` API / Viewer UI を段階実装予定
- production コードは未着手（7D-0 は設計文書のみ）

### 未実装（v1 非範囲を含む）

- 自動再収集 / batch 収集 / Figma Import 実装 / visual diff / annotation / viewport config
...
| `figma-frame-import.md` | Figma Frame Import 設計（Phase 7D-0） |
| `README.md` | 本契約文書 |
