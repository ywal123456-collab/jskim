...
### 実装済み（phase 7C-1A-1 Device Capture core）
...
- `inputRevision`（プロジェクト既知入力の SHA-256）と `imageRevision`（PNG bytes）で stale / current を判定。外部 API・日時・乱数などの runtime 同一性は保証しない
- generation 画像 + `meta.json` の atomic commit。失敗時は既存 Capture を維持。同一結果は no-op
- 既存 `jskim spec collect` は Device Capture を自動実行しない。HTTP Capture API と Viewer の Live/PC/SP タブは未実装（7C-1A-2 / 7C-1A-3）

### 未実装（v1 非範囲を含む）

- Device Capture POST API / Viewer Live・PC・SP タブ / 再収集 UI
