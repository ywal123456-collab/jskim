...
- PUT の item ID 集合 validation: **最新 collected item ID ⊆ 新 item ID**（manual-only 削除と新規追加は許可。collected 削除は `SPEC_DESCRIPTION_COLLECTED_ITEM_DELETE_NOT_ALLOWED`）
...
- 本 phase の対象外（未実装）: collected 項目の除外・接続解除、drag-drop 並び替え、画面の rename / archive / delete、Figma・Reference Image 連携
