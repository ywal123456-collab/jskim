...
### schemaVersion と `itemOrder`（phase 7B-2A）

- `schemaVersion: "1.0"` は `docs/screen-spec/schema/description-spec.v1.schema.json`。`itemOrder` を持ちません
...
- 読込は両方に対応します。新規作成（POST）と Viewer での保存（PUT）は常に `1.1` を書き出します
- 既存の `1.0` ファイルは、保存操作（Viewer PUT または Collector による item 追加）が起きるまで自動で書き換えません（lazy migration）
...
| item の追加・削除が無い `1.0` ファイル | `1.0` のまま維持（不要な `1.1` upgrade をしない） |
...
- 読込は `1.0` / `1.1` の両方に対応。Viewer での保存（PUT）時に **lazy migration** で `1.1` へ書き出す（保存操作が起きるまで既存 `1.0` ファイルは書き換えない）
- 新規画面の POST 作成（`jskim spec dev` の「＋ 画面を作成」）は `schemaVersion "1.1"` + `itemOrder: []` で作成する
- Viewer に「＋ 項目を追加」ボタンを追加し、項目 ID / 項目名 / 種類 / 説明 / 備考を入力するダイアログから項目を新規作成できる（削除・複製・drag-drop 並び替えは未実装）
- `ItemDescriptionTable` に上下ボタンを追加し、項目の並び替え（`itemOrder` の更新）ができる
- PUT の item ID 集合 validation を変更: 既存 item ID の**変更・削除は拒否**するが、**新規 ID の追加は許可**する（`oldIds ⊆ newIds`）
- `itemOrder` と `items` のキー集合は一致必須（bijection）。不一致は 400
- Collector（`jskim spec collect`）は人が並べた `itemOrder` を維持し、新規に見つかった ID を DOM 出現順で末尾に追加する。item の追加・削除が無い場合は `1.0` のまま維持し、不要な rewrite をしない
- 本 phase の対象外（未実装）: 項目の削除・複製・drag-drop 並び替え、画面の rename / archive / delete、Figma・Reference Image 連携
...
- `screenId` は読み取り専用。`itemId` の集合は phase 7B-2A 以降、新規追加のみ許可（既存 ID の変更・削除は拒否。詳細は phase 7B-2A の項目を参照）
