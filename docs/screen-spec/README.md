...
- Feature Group / ローカル版管理（Phase **7E-1〜7E-4A**: domain + `jskim spec version` CLI 実装済み・Viewer UI/Revision API 未提供。設計: [local-version-control.md](./local-version-control.md)）
- ユーザー CLI / Revision API / Viewer 改訂履歴 / merge / Excel Export / Remote（未実装）
...
12. 画面中心 Feature Group + ローカル版管理（Phase 7E-1〜5・設計済み / 未実装）
13. local merge（Phase 7E-6・設計済み / 未実装）
14. version-aware Excel Export（Phase 7F・設計済み / 未実装）
15. Remote Provider Framework（Phase 7G・設計のみ。初期は non-ff 拒否）
16. 必要になった場合のみ Vite HMR / incremental collect
...
| `local-version-control.md` | 画面中心モデル・Feature Group・ローカル版管理（7E-1〜7E-4A: domain + CLI 実装済み / Viewer UI 未提供） |
