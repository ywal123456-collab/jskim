...
| package | 役割 |
|---------|------|
| `@ywal123456/jskim` | 静的 HTML build / watch / serve / dev / `spec collect` / `spec build` |
...
jskim dev sample
# /  → 実装画面
# /spec/ → 画面設計書 SPA（先に spec collect / spec build が必要）
```

`jskim spec collect` は preserve ビルド → 一時サーバー → companion collector の順で実行します。
`jskim dev` は Screen Spec を自動 build しません。先に `jskim spec collect` と `jskim spec build` を実行してください。
...
- `jskim dev` 開始時の Screen Spec 自動 collect / 自動 build
...
10. Vite middleware / Screen Spec watch（phase 4B）
